...
import { StepList } from "@/components/install/StepList";
import { cn } from "@/lib/utils";
import { detectInstallPlatform, isStandaloneDisplayMode, type InstallPlatformInfo } from "@/lib/device";
...
export function InstallClient() {
  ...
  const installFeedback = useMemo(() => {
    ...
    if (installOutcome === "error") return "Non siamo riusciti ad avviare l’installazione. Usa i passaggi manuali.";
    ...
    return (
      <div className="flex min-h-screen items-center justify-center bg-[#0f1024] px-6 py-12 text-white">
        <div className="w-full max-w-xl rounded-3xl border border-[#3f3a74]/65 bg-[#171736] p-8 text-center shadow-[0_30px_80px_rgba(5,4,24,0.55)]">
          ...
          <h1 className="text-2xl font-extrabold text-white">Installa l’app dal tuo telefono</h1>
          <p className="mx-auto mt-3 max-w-md text-sm leading-relaxed text-[#c2bddf]">
            Questa procedura è ottimizzata per mobile. Scansiona il QR code con il tuo telefono per installare l’app.
          </p>
          ...
          <p className="mt-5 text-xs font-medium uppercase tracking-[0.1em] text-[#b7b1d8]">
            Scansiona con il tuo telefono per installare l’app
          </p>
        </div>
      </div>
    );
    ...
    <div className="min-h-screen bg-[#0f1024] text-white">
      ...
      <main className="relative z-10 mx-auto flex min-h-screen w-full max-w-md flex-col gap-5 px-5 py-7">
        <header className="rounded-3xl border border-[#4a4488]/60 bg-[#19183b]/85 p-6 shadow-[0_20px_50px_rgba(5,4,20,0.45)]">
          ...
          <h1 className="text-2xl font-extrabold leading-tight">Installa l’app sul tuo telefono</h1>
          <p className="mt-3 text-sm leading-relaxed text-[#ccc7e8]">
            Accedi più velocemente direttamente dalla schermata Home, con un’esperienza più fluida e immediata.
          </p>
        </header>
        ...
        {isInstalled ? (
          <section className="rounded-3xl border border-emerald-400/35 bg-emerald-500/10 p-5">
            <div className="flex items-start gap-3">
              ...
              <div>
                <h2 className="text-base font-bold text-emerald-100">L’app è già installata</h2>
                <p className="mt-1 text-sm leading-relaxed text-emerald-100/85">
                  Perfetto, puoi continuare subito nell’app senza passare dal browser.
                </p>
              </div>
            </div>
            <Link
          ...
          <>
            <section className="rounded-3xl border border-[#4a4488]/60 bg-[#171735] p-5">
              ...
              <p className="mt-2 text-sm leading-relaxed text-[#d6d1ee]">
                Su iPhone: apri questa pagina in Safari e tocca Condividi per aggiungere l’app alla schermata Home.
              </p>
              <button
                type="button"
          ...
          <>
            <section className="rounded-3xl border border-amber-300/35 bg-amber-400/10 p-5">
              ...
              <p className="mt-2 text-sm leading-relaxed text-amber-100/90">
                ...
                correttamente l’installazione PWA.
              </p>
            </section>
            <StepList title="Poi fai così in Safari" steps={IOS_STEPS} />
          </>
        ) : null}
          ...
          <>
            <section className="rounded-3xl border border-[#4a4488]/60 bg-[#171735] p-5">
              <div className="flex items-start gap-3">
                ...
                <div>
                  <p className="text-xs font-semibold uppercase tracking-[0.11em] text-[#b7b1d8]">Compatibilità</p>
                  <p className="mt-2 text-sm leading-relaxed text-[#d6d1ee]">
                    Non riusciamo a identificare il browser con precisione. Apri questa pagina in Safari (iPhone) o
                    Chrome (Android) per una procedura più rapida.
                  </p>
                </div>
              </div>
            </section>
        ...
        <footer className="mt-auto pb-1 pt-2 text-center">
          <p className="text-[11px] font-medium uppercase tracking-[0.1em] text-[#a8a2cb]">
            Esperienza dedicata solo all’installazione PWA
          </p>
          <Link
            href="/market"
