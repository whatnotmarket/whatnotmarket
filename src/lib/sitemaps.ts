...
const getCachedListingRecords = unstable_cache(fetchListingRecords, ["sitemap:listings:v2"], {
  revalidate: SITEMAP_CACHE_SECONDS,
  tags: ["sitemap:index", "sitemap:listings", "sitemap:categories"],
});

const getCachedCategoryRecords = unstable_cache(fetchCategoryRecords, ["sitemap:categories:v2"], {
  revalidate: SITEMAP_CACHE_SECONDS,
  tags: ["sitemap:index", "sitemap:categories"],
});

const getCachedUserRecords = unstable_cache(fetchUserRecords, ["sitemap:users:v2"], {
  revalidate: SITEMAP_CACHE_SECONDS,
...
export async function getSitemapIndexEntries(): Promise<SitemapUrlEntry[]> {
  const [listingChunks, categories, users, pages] = await Promise.all([
  ...
  entries.push({
    ...
    lastmod: maxLastmod(categories),
  });
  entries.push({
    loc: absoluteUrl("/sitemaps/users.xml"),
    lastmod: maxLastmod(users),
  });
  entries.push({
