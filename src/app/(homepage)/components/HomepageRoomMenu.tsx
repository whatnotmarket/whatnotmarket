    ...
    <div className="relative" ref={containerRef}>
      ...
      >
        <span className="flex items-center gap-2">
          <span className="grid h-6 w-6 place-items-center text-white">{renderRoomIcon(activeRoom, "h-3.5 w-3.5")}</span>
          <span>{activeRoomLabel}</span>
        </span>
        <ChevronDown
          className={cn("ml-2 h-4 w-4 text-white transition-transform", isOpen ? "rotate-180" : "")}
        />
      </button>
                ...
                <li key={room.slug}>
                  <button
                    ...
                    className={cn(
                      ...
                      isActive ? "bg-[#161923] text-white" : "text-white hover:bg-[#2E3547]"
                    )}
                  >
                    <span className="grid h-6 w-6 shrink-0 place-items-center text-white">
