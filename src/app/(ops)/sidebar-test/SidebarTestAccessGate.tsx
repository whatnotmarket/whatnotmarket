...
export function SidebarTestAccessGate() {
  ...
  async function unlock() {
    ...
    try {
      const response = await fetch("/api/sidebar-test/access", {
        method: "POST",
      ...
      setError("Unable to unlock sidebar test right now.");
    } finally {
      setSubmitting(false);
    }
  }
  ...
  return (
    <main className="min-h-screen bg-[#c7c3df] px-4 py-12">
      <div className="mx-auto mt-16 w-full max-w-[430px] rounded-[20px] border border-black/40 bg-[#06070a] p-6 text-white shadow-[0_10px_24px_rgba(0,0,0,0.35)]">
        <h1 className="text-2xl font-semibold">Sidebar Test Private Access</h1>
        <p className="mt-2 text-sm text-white/70">This page is temporarily protected while we iterate on UI behavior.</p>
        <div className="mt-5 space-y-3">
          ...
          >
            {submitting ? "Unlocking..." : "Unlock sidebar test"}
          </Button>
        </div>
      </div>
    </main>
