...
async function getTrendingSnapshot(client: AnySupabaseClient): Promise<TrendingSnapshot> {
  ...
  for (const request of requests) {
    items.push({
      ...
      title: request.title,
      description: `${request.category} Â· ${formatBudget(request.budget_min, request.budget_max)}`,
  ...
  for (const seller of topSellers) {
    ...
    items.push({
      ...
      description: `Best Sellers of the Week Â· ${Number(seller.total_deals || 0)} deals`,
...
async function getLiveQueryItems(
  ...
  for (const profile of profiles) {
    ...
    items.push({
      id: `user-${profile.id}`,
      title: label,
      description: `${isVerified ? "Venditore verificato" : "Utente"} Â· @${handle}`,
  ...
  for (const category of categories) {
    items.push({
      ...
      title: category.name,
      description: `Category Â· ${category.id}`,
      href: `/category/${category.id}`,
      group: "Categories",
      icon: "tag",
      entityType: "category",
