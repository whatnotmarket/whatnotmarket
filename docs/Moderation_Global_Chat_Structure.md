# Moderazione Global Chat — Struttura completa (DB, API, Realtime, UI)

## Panoramica
- Chat globale con moderazione realtime: delete/ban/mute, slow‑mode per stanza, chiusura stanza temporanea o permanente.
...
- public.global_chat_messages
  ...
  - RLS: SELECT pubblico con `is_deleted=false` (policy “not deleted”); insert/update via API.
...
- Messaggi (lettura pubblica filtrata):
  - Policy: “Anyone can read global chat messages not deleted” → `USING (is_deleted = false)`
...
- POST /api/admin/dashboard/public-chat
  - Azioni:
    ...
    - `ban_message` → `is_deleted=true` + log
...
- Mute attivo: rifiuta post, logga “MUTE_ACTIVE”
- Slow‑mode attivo: se l’utente ha post recenti in finestra Xs, rifiuta “SLOW_MODE” con secondi residui
- Ruoli stanza:
  - Sell Services: solo seller avvia thread; buyer può rispondere
  - Buy Services: solo buyer avvia thread; seller può rispondere
- Chiusura stanza:
  - `closed_until > now` → rifiuta post “ROOM_CLOSED”

## Realtime lato client (utente)
- Messaggi: subscribe INSERT/UPDATE; se `is_deleted=true` → rimozione immediata
...
- Banner inline (trasparenti, minimal, allineati):
  - Banned: rosso “You are banned to write here”
  - Muted: “You are muted to write here”
  - Slow‑mode: icona + “Slow mode attivo (Xs)”
  - Closed permanente: “This chat is closed”; closed temporaneo: “This chat is closed for Xs”
- Input:
...
- Top controls:
  ...
  - Slow‑mode quick buttons: OFF, 10s, 30s, 60s
  - Close room (permanente) / Open room
  - Reload
- Stream messaggi (colonna sinistra):
  - Card con Username, Lingua, Timestamp, Testo; deleted → “Message removed” con stile grigio/line‑through
...
- Rollout:
  ...
  - Configura policy e variabili d’ambiente
